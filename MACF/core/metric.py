...
class MetricsTop():
    ...
    def __eval_mosei_regression(self, y_pred, y_true, exclude_zero=False):
        test_preds = y_pred.view(-1).cpu().detach().numpy()   # 将预测值转换为一维
        test_truth = y_true.view(-1).cpu().detach().numpy()

        test_preds_a7 = np.clip(test_preds, a_min=-3., a_max=3.)   # 其中的元素限制取值为-3 3
        ...
        corr = np.corrcoef(test_preds, test_truth)[0][1]  #计算协方差矩阵，取得是预测值与真实值的相关性，因为它们都是一个维度，所以是2 x 2
        mult_a7 = self.__multiclass_acc(test_preds_a7, test_truth_a7) #计算acc-7
        mult_a5 = self.__multiclass_acc(test_preds_a5, test_truth_a5) #计算acc-5
        mult_a3 = self.__multiclass_acc(test_preds_a3, test_truth_a3) #计算acc-3
