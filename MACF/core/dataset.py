...
class MMDataset(Dataset):
    ...
    def __init_mosi(self):
        with open(self.args.dataPath, 'rb') as f:
            data = pickle.load(f)
        # PKL file is a format for saving files in Python. If opened directly, it will display a bunch of serialized files (binary files).
        # Commonly used to store models trained by neural networks or various data that needs to be stored.
        # The above code reads data from a pickle format file and converts it to a Python type.
        ...
        print("vision.shape in mosi",self.vision.shape)
        self.audio = data[self.mode]['audio'].astype(np.float32)
        print("audio.shape in mosi", self.audio.shape)
